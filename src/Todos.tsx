...
type TodoInputProps = { onAdd: addTodoFunctionType }
type TodoListProps = {
  todos: Todo[];
  onRemove: removeTodoFunctionType;
  title: string;
  setTodos: React.Dispatch<React.SetStateAction<Todo[]>>;
}

type removeTodoFunctionType = (id: number) => void;
type addTodoFunctionType = (title: string, owner: string) => void;
...
function App() {
  ...
  const addTodo: addTodoFunctionType = (title, owner) => {
    const newTodo = {
      id: todos.length + 1,
      title,
      owner,
  ...
  const removeTodo: removeTodoFunctionType = (id) => {
    // added latestTodo removal functionality
    if (latestTodo && latestTodo.id === id) {
      setLatestTodo(null);
    }
  ...
  return (
    ...
    >
      ...
      {latestTodo && (
        ...
        >
          <h2>Latest Todo</h2>
          <Todo todo={latestTodo} onRemove={removeTodo} setTodos={setTodos} />
        </div>
      )}
      <div
        style={{
...
function TodoList({ todos, onRemove, title, setTodos }: TodoListProps ) {
  return (
    <div
      style={{
        marginBottom: "20px",
...
function Todo({ todo, onRemove, setTodos } : { todo: Todo, onRemove: removeTodoFunctionType }) {
  return (
    <div
      style={{
        display: "flex",
